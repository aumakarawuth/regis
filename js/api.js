...
const API = {
  ...
  async getPrograms() {
    const [{ data: levelRows, error: levelError }, { data: branches, error }] = await Promise.all([
      // Fetched separately (ordered by code — LV1/ปวช. before LV2/ปวส.)
      // rather than derived from "first branch seen", so the level
      // order can never depend on branch sort_order or query order.
    ...
    const levels = (levelRows || []).map(l => ({ id: l.code, name: l.name }));
    const branchList = (branches || []).map(b => {
      ...
      return {
        ...
        showWorkLocation: !!b.show_work_location,
