-- ============================================================
-- 0015_study_category.sql
--
-- apply.html adds a new "หมวดการเรียน" step after the applicant picks
-- a branch: เรียน จ-ศ (in-classroom) or เรียนไปทำงานไป (study while
-- working), and for branches that offer a work-study track (retail
-- business), a further กรุงเทพ/ต่างจังหวัด work-location choice.
-- Neither had a column to land in — adds both to enrollments and
-- wires them through submit_application().
-- ============================================================

alter table enrollments add column if not exists study_category text;
alter table enrollments add column if not exists work_location text;
...
begin
  ...
  insert into enrollments (student_id, program_round_id, application_no, status, study_category, work_location)
  values (
    v_student_id,
    v_program_round_id,
    v_application_no,
    'pending',
    nullif(payload->'program'->>'studyCategory', ''),
    nullif(payload->'program'->>'workLocation', '')
  );
